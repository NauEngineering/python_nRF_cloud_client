...
class C2COverages(BaseModel):
    ...
    ground_fix: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, alias="GROUND_FIX")
    type: StrictStr
    __properties: ClassVar[List[str]] = ["AGPS", "PGPS", "REVERSE_GEO", "GROUND_FIX", "type"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['C2C']):
            raise ValueError("must be one of enum values ('C2C')")
        return value
