...
class PaginatedResultDeviceTagOrJSONataTransformedResponse(BaseModel):
    """
    PaginatedResultDeviceTagOrJSONataTransformedResponse
    """ # noqa: E501
    items: List[PaginatedResultDeviceTagOrJSONataTransformedResponseItemsInner]
    total: Optional[Annotated[int, Field(strict=True, ge=0)]] = Field(default=None, description="Reflects the total results returned by the query, which may be less than the total number of items available. If the response contains a `pageNextToken` value, you can supply the `pageNextToken` in the next request to get more results. The maximum value of `total` is the page limit of the request, or ten pages if no page limit is provided.")
    page_next_token: Optional[StrictStr] = Field(default=None, description="Token used to retrieve the next page of items in the list. Present in a response only if the total available results exceeds the specified limit on a page. This token does not change between requests. When supplying as a request parameter, use URL-encoding.", alias="pageNextToken")
    __properties: ClassVar[List[str]] = ["items", "total", "pageNextToken"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "items": [PaginatedResultDeviceTagOrJSONataTransformedResponseItemsInner.from_dict(_item) for _item in obj["items"]] if obj.get("items") is not None else None,
            "total": obj.get("total"),
            "pageNextToken": obj.get("pageNextToken")
        })
        return _obj
