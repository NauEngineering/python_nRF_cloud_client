...
GATEWAYDEVICESUBTYPE_ANY_OF_SCHEMAS = ["HardGatewaySubType", "SoftGatewayType"]

class GatewayDeviceSubType(BaseModel):
    """
    GatewayDeviceSubType
    """

    # data type: SoftGatewayType
    anyof_schema_1_validator: Optional[SoftGatewayType] = None
    # data type: HardGatewaySubType
    anyof_schema_2_validator: Optional[HardGatewaySubType] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[HardGatewaySubType, SoftGatewayType]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "HardGatewaySubType", "SoftGatewayType" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = GatewayDeviceSubType.model_construct()
        error_messages = []
        # validate data type: SoftGatewayType
        if not isinstance(v, SoftGatewayType):
            error_messages.append(f"Error! Input type `{type(v)}` is not `SoftGatewayType`")
        else:
            return v

        # validate data type: HardGatewaySubType
        if not isinstance(v, HardGatewaySubType):
            error_messages.append(f"Error! Input type `{type(v)}` is not `HardGatewaySubType`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in GatewayDeviceSubType with anyOf schemas: HardGatewaySubType, SoftGatewayType. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[SoftGatewayType] = None
        try:
            instance.actual_instance = SoftGatewayType.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[HardGatewaySubType] = None
        try:
            instance.actual_instance = HardGatewaySubType.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into GatewayDeviceSubType with anyOf schemas: HardGatewaySubType, SoftGatewayType. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], HardGatewaySubType, SoftGatewayType]]:
