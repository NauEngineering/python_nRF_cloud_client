...
CREATEJOBINPUTIDORIDENTIFIER_ANY_OF_SCHEMAS = ["CreateJobInputDeviceIdentifiers", "CreateJobInputDeviceIds"]

class CreateJobInputIdOrIdentifier(BaseModel):
    """
    CreateJobInputIdOrIdentifier
    """

    # data type: CreateJobInputDeviceIds
    anyof_schema_1_validator: Optional[CreateJobInputDeviceIds] = None
    # data type: CreateJobInputDeviceIdentifiers
    anyof_schema_2_validator: Optional[CreateJobInputDeviceIdentifiers] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[CreateJobInputDeviceIdentifiers, CreateJobInputDeviceIds]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "CreateJobInputDeviceIdentifiers", "CreateJobInputDeviceIds" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = CreateJobInputIdOrIdentifier.model_construct()
        error_messages = []
        # validate data type: CreateJobInputDeviceIds
        if not isinstance(v, CreateJobInputDeviceIds):
            error_messages.append(f"Error! Input type `{type(v)}` is not `CreateJobInputDeviceIds`")
        else:
            return v

        # validate data type: CreateJobInputDeviceIdentifiers
        if not isinstance(v, CreateJobInputDeviceIdentifiers):
            error_messages.append(f"Error! Input type `{type(v)}` is not `CreateJobInputDeviceIdentifiers`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in CreateJobInputIdOrIdentifier with anyOf schemas: CreateJobInputDeviceIdentifiers, CreateJobInputDeviceIds. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[CreateJobInputDeviceIds] = None
        try:
            instance.actual_instance = CreateJobInputDeviceIds.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[CreateJobInputDeviceIdentifiers] = None
        try:
            instance.actual_instance = CreateJobInputDeviceIdentifiers.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into CreateJobInputIdOrIdentifier with anyOf schemas: CreateJobInputDeviceIdentifiers, CreateJobInputDeviceIds. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], CreateJobInputDeviceIdentifiers, CreateJobInputDeviceIds]]:
