...
GETLOCATIONFROMCELLSORWIFINETWORKSREQUEST_ANY_OF_SCHEMAS = ["DeprecatedGroundFixRequest", "GroundFixRequest"]

class GetLocationFromCellsOrWifiNetworksRequest(BaseModel):
    """
    GetLocationFromCellsOrWifiNetworksRequest
    """

    # data type: GroundFixRequest
    anyof_schema_1_validator: Optional[GroundFixRequest] = None
    # data type: DeprecatedGroundFixRequest
    anyof_schema_2_validator: Optional[DeprecatedGroundFixRequest] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[DeprecatedGroundFixRequest, GroundFixRequest]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "DeprecatedGroundFixRequest", "GroundFixRequest" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = GetLocationFromCellsOrWifiNetworksRequest.model_construct()
        error_messages = []
        # validate data type: GroundFixRequest
        if not isinstance(v, GroundFixRequest):
            error_messages.append(f"Error! Input type `{type(v)}` is not `GroundFixRequest`")
        else:
            return v

        # validate data type: DeprecatedGroundFixRequest
        if not isinstance(v, DeprecatedGroundFixRequest):
            error_messages.append(f"Error! Input type `{type(v)}` is not `DeprecatedGroundFixRequest`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in GetLocationFromCellsOrWifiNetworksRequest with anyOf schemas: DeprecatedGroundFixRequest, GroundFixRequest. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[GroundFixRequest] = None
        try:
            instance.actual_instance = GroundFixRequest.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[DeprecatedGroundFixRequest] = None
        try:
            instance.actual_instance = DeprecatedGroundFixRequest.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into GetLocationFromCellsOrWifiNetworksRequest with anyOf schemas: DeprecatedGroundFixRequest, GroundFixRequest. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], DeprecatedGroundFixRequest, GroundFixRequest]]:
