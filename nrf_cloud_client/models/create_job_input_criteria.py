...
class CreateJobInputCriteria(BaseModel):
    """
    CreateJobInputCriteria
    """ # noqa: E501
    bundle_id: CreateJobInputTagsBundleId = Field(alias="bundleId")
    auto_apply: Optional[StrictBool] = Field(default=True, description="If set to false the job will not be applied immediately, but instead will be left in the \"CREATED\" status. The job can later be applied using POST /fota-jobs/{jobId}/apply", alias="autoApply")
    name: Optional[Annotated[str, Field(strict=True, max_length=64)]] = Field(default=None, description="Friendly name for this job")
    description: Optional[Annotated[str, Field(strict=True, max_length=1024)]] = Field(default=None, description="Short description for this job")
    tags: Optional[Annotated[List[StrictStr], Field(min_length=0, max_length=100)]] = Field(default=None, description="A list of valid tags in which a device must contain one of for it to be eligible for this job.")
    device_ids: Optional[Annotated[List[Annotated[str, Field(strict=True)]], Field(min_length=0, max_length=100)]] = Field(default=None, description="List of valid device identifiers for this job.", alias="deviceIds")
    firmware_type: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, description="Currently not supported. Firmware type a device must support for it to be eligible for this job.", alias="firmwareType")
    firmware_versions: Optional[Annotated[List[StrictStr], Field(min_length=0, max_length=5)]] = Field(default=None, description="Currently not supported. A list of firmware versions for the given firmware type that a device must have for it to be eligible for this job.", alias="firmwareVersions")
    device_identifiers: Optional[Annotated[List[Annotated[str, Field(strict=True)]], Field(min_length=0, max_length=100)]] = Field(default=None, description="A list of device identifiers assigned to this job. Deprecated. Use deviceIds instead.", alias="deviceIdentifiers")
    schedule: Optional[JobSchedule] = None
    target_list_type: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, description="Currently not supported. Determine whether a target list is LOCKED or UNLOCKED. An UNLOCKED target list allows devices that are newly eligible for the job (based on target criteria) to receive an execution. A LOCKED target prevents devices that are newly eligible to receive an execution.", alias="targetListType")
    __properties: ClassVar[List[str]] = ["bundleId", "autoApply", "name", "description", "tags", "deviceIds", "firmwareType", "firmwareVersions", "deviceIdentifiers", "schedule", "targetListType"]

    @field_validator('firmware_type')
    def firmware_type_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if value is None:
            return value

        if not re.match(r"^(APP|BOOT|MODEM)$", value):
            raise ValueError(r"must validate the regular expression /^(APP|BOOT|MODEM)$/")
        return value

    @field_validator('target_list_type')
    def target_list_type_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if value is None:
            return value

        if not re.match(r"^LOCKED|UNLOCKED$", value):
            raise ValueError(r"must validate the regular expression /^LOCKED|UNLOCKED$/")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "bundleId": CreateJobInputTagsBundleId.from_dict(obj["bundleId"]) if obj.get("bundleId") is not None else None,
            "autoApply": obj.get("autoApply") if obj.get("autoApply") is not None else True,
            "name": obj.get("name"),
            "description": obj.get("description"),
            "tags": obj.get("tags"),
            "deviceIds": obj.get("deviceIds"),
            "firmwareType": obj.get("firmwareType"),
            "firmwareVersions": obj.get("firmwareVersions"),
            "deviceIdentifiers": obj.get("deviceIdentifiers"),
            "schedule": JobSchedule.from_dict(obj["schedule"]) if obj.get("schedule") is not None else None,
            "targetListType": obj.get("targetListType")
        })
        return _obj
