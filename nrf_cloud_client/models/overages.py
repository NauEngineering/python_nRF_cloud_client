...
OVERAGES_ANY_OF_SCHEMAS = ["C2COverages", "D2COverages"]
...
class Overages(BaseModel):
    ...
    # data type: C2COverages
    anyof_schema_1_validator: Optional[C2COverages] = None
    # data type: D2COverages
    anyof_schema_2_validator: Optional[D2COverages] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[C2COverages, D2COverages]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "C2COverages", "D2COverages" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = Overages.model_construct()
        error_messages = []
        # validate data type: C2COverages
        if not isinstance(v, C2COverages):
            error_messages.append(f"Error! Input type `{type(v)}` is not `C2COverages`")
        else:
            return v

        # validate data type: D2COverages
        if not isinstance(v, D2COverages):
            error_messages.append(f"Error! Input type `{type(v)}` is not `D2COverages`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in Overages with anyOf schemas: C2COverages, D2COverages. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[C2COverages] = None
        try:
            instance.actual_instance = C2COverages.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[D2COverages] = None
        try:
            instance.actual_instance = D2COverages.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into Overages with anyOf schemas: C2COverages, D2COverages. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], C2COverages, D2COverages]]:
