...
GENERICDEVICESUBTYPE_ANY_OF_SCHEMAS = ["NonGatewayIPDeviceSubType", "str"]

class GenericDeviceSubType(BaseModel):
    """
    GenericDeviceSubType
    """

    # data type: NonGatewayIPDeviceSubType
    anyof_schema_1_validator: Optional[NonGatewayIPDeviceSubType] = None
    # data type: str
    anyof_schema_2_validator: Optional[StrictStr] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[NonGatewayIPDeviceSubType, str]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "NonGatewayIPDeviceSubType", "str" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = GenericDeviceSubType.model_construct()
        error_messages = []
        # validate data type: NonGatewayIPDeviceSubType
        if not isinstance(v, NonGatewayIPDeviceSubType):
            error_messages.append(f"Error! Input type `{type(v)}` is not `NonGatewayIPDeviceSubType`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in GenericDeviceSubType with anyOf schemas: NonGatewayIPDeviceSubType, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[NonGatewayIPDeviceSubType] = None
        try:
            instance.actual_instance = NonGatewayIPDeviceSubType.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into GenericDeviceSubType with anyOf schemas: NonGatewayIPDeviceSubType, str. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], NonGatewayIPDeviceSubType, str]]:
