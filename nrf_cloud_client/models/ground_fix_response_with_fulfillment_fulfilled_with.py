...
GROUNDFIXRESPONSEWITHFULFILLMENTFULFILLEDWITH_ANY_OF_SCHEMAS = ["GroundFixServiceType", "str"]

class GroundFixResponseWithFulfillmentFulfilledWith(BaseModel):
    """
    GroundFixResponseWithFulfillmentFulfilledWith
    """

    # data type: GroundFixServiceType
    anyof_schema_1_validator: Optional[GroundFixServiceType] = None
    # data type: str
    anyof_schema_2_validator: Optional[StrictStr] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[GroundFixServiceType, str]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "GroundFixServiceType", "str" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = GroundFixResponseWithFulfillmentFulfilledWith.model_construct()
        error_messages = []
        # validate data type: GroundFixServiceType
        if not isinstance(v, GroundFixServiceType):
            error_messages.append(f"Error! Input type `{type(v)}` is not `GroundFixServiceType`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in GroundFixResponseWithFulfillmentFulfilledWith with anyOf schemas: GroundFixServiceType, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[GroundFixServiceType] = None
        try:
            instance.actual_instance = GroundFixServiceType.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into GroundFixResponseWithFulfillmentFulfilledWith with anyOf schemas: GroundFixServiceType, str. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], GroundFixServiceType, str]]:
