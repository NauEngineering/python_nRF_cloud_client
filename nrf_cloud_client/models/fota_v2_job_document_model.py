...
class FotaV2JobDocumentModel(BaseModel):
    """
    FotaV2JobDocumentModel
    """ # noqa: E501
    firmware_type: FirmwareTypes = Field(alias="firmwareType")
    file_size: Union[StrictFloat, StrictInt] = Field(alias="fileSize")
    host: StrictStr
    path: StrictStr
    version: StrictStr
    bundle_id: Annotated[str, Field(strict=True)] = Field(description="Unique bundle identifier", alias="bundleId")
    __properties: ClassVar[List[str]] = ["firmwareType", "fileSize", "host", "path", "version", "bundleId"]

    @field_validator('bundle_id')
    def bundle_id_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^(APP|MODEM|BOOT|SOFTDEVICE|BOOTLOADER|MDM_FULL|SMP|CUSTOM)\*[0-9a-zA-Z]{8}\*.*$", value):
            raise ValueError(r"must validate the regular expression /^(APP|MODEM|BOOT|SOFTDEVICE|BOOTLOADER|MDM_FULL|SMP|CUSTOM)\*[0-9a-zA-Z]{8}\*.*$/")
        return value
