...
PICKINVOICELINEITEMNONOVERAGEEXCLUDEKEYOFINVOICELINEITEMNONOVERAGETYPESERVICEID_ANY_OF_SCHEMAS = ["CommercialServices", "GroundFixLocationService", "str"]

class PickInvoiceLineItemNonOverageExcludeKeyofInvoiceLineItemNonOverageTypeServiceId(BaseModel):
    """
    PickInvoiceLineItemNonOverageExcludeKeyofInvoiceLineItemNonOverageTypeServiceId
    """

    # data type: CommercialServices
    anyof_schema_1_validator: Optional[CommercialServices] = None
    # data type: GroundFixLocationService
    anyof_schema_2_validator: Optional[GroundFixLocationService] = None
    # data type: str
    anyof_schema_3_validator: Optional[StrictStr] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[CommercialServices, GroundFixLocationService, str]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "CommercialServices", "GroundFixLocationService", "str" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = PickInvoiceLineItemNonOverageExcludeKeyofInvoiceLineItemNonOverageTypeServiceId.model_construct()
        error_messages = []
        # validate data type: CommercialServices
        if not isinstance(v, CommercialServices):
            error_messages.append(f"Error! Input type `{type(v)}` is not `CommercialServices`")
        else:
            return v

        # validate data type: GroundFixLocationService
        if not isinstance(v, GroundFixLocationService):
            error_messages.append(f"Error! Input type `{type(v)}` is not `GroundFixLocationService`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in PickInvoiceLineItemNonOverageExcludeKeyofInvoiceLineItemNonOverageTypeServiceId with anyOf schemas: CommercialServices, GroundFixLocationService, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[CommercialServices] = None
        try:
            instance.actual_instance = CommercialServices.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[GroundFixLocationService] = None
        try:
            instance.actual_instance = GroundFixLocationService.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into PickInvoiceLineItemNonOverageExcludeKeyofInvoiceLineItemNonOverageTypeServiceId with anyOf schemas: CommercialServices, GroundFixLocationService, str. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], CommercialServices, GroundFixLocationService, str]]:
