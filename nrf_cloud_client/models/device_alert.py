...
class DeviceAlert(BaseModel):
    ...
    DeviceAlert
    """ # noqa: E501
    id: Annotated[str, Field(strict=True)] = Field(description="Universally unique identifier")
    device_id: Annotated[str, Field(strict=True)] = Field(description="This is the canonical device id used in the device certificate, and as the MQTT client id.", alias="deviceId")
    created_at: Optional[datetime] = Field(default=None, description="HTML-encoded ISO-8601 date-time string denoting the start or end of a date range. If the string includes only a date, the time is the beginning of the day (00:00:00).", alias="createdAt")
    received_at: datetime = Field(description="HTML-encoded ISO-8601 date-time string denoting the start or end of a date range. If the string includes only a date, the time is the beginning of the day (00:00:00).", alias="receivedAt")
    status_changed_at: Optional[datetime] = Field(default=None, description="HTML-encoded ISO-8601 date-time string denoting the start or end of a date range. If the string includes only a date, the time is the beginning of the day (00:00:00).", alias="statusChangedAt")
    status_changed_by: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, description="Universally unique identifier", alias="statusChangedBy")
    status: AlertStatus
    original_message: Dict[str, Any] = Field(description="The original message sent from the device that triggered the alert.", alias="originalMessage")
    __properties: ClassVar[List[str]] = ["id", "deviceId", "createdAt", "receivedAt", "statusChangedAt", "statusChangedBy", "status", "originalMessage"]
    ...
    @field_validator('status_changed_by')
    def status_changed_by_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if value is None:
            return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "originalMessage": obj.get("originalMessage")
        })
        return _obj
