...
class FetchFOTAJobExecution200ResponseAnyOf(BaseModel):
    """
    FetchFOTAJobExecution200ResponseAnyOf
    """ # noqa: E501
    job_document: FetchFOTAJobExecution200ResponseAnyOfJobDocument = Field(alias="jobDocument")
    status_detail: StrictStr = Field(alias="statusDetail")
    status: FotaV2JobExecutionEvents
    ...
    job_id: Annotated[str, Field(strict=True)] = Field(description="Universally unique identifier", alias="jobId")
    execution_id: Annotated[str, Field(strict=True)] = Field(description="Universally unique identifier", alias="executionId")
    tenant_id: Annotated[str, Field(strict=True)] = Field(description="Universally unique identifier", alias="tenantId")
    __properties: ClassVar[List[str]] = ["jobDocument", "statusDetail", "status", "lastUpdatedAt", "createdAt", "deviceId", "jobId", "executionId", "tenantId"]
    ...
    def execution_id_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^[a-f0-9]{8}-?[a-f0-9]{4}-?[a-f0-9]{4}-?[a-f0-9]{4}-?[a-f0-9]{12}$", value):
            raise ValueError(r"must validate the regular expression /^[a-f0-9]{8}-?[a-f0-9]{4}-?[a-f0-9]{4}-?[a-f0-9]{4}-?[a-f0-9]{12}$/")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of job_document
        if self.job_document:
            _dict['jobDocument'] = self.job_document.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "jobDocument": FetchFOTAJobExecution200ResponseAnyOfJobDocument.from_dict(obj["jobDocument"]) if obj.get("jobDocument") is not None else None,
