...
class BLEDevice(BaseModel):
    ...
    BLEDevice
    """ # noqa: E501
    id: Annotated[str, Field(strict=True)] = Field(description="This is the canonical device id used in the device certificate, and as the MQTT client id.")
    name: Annotated[str, Field(strict=True)] = Field(description="Friendly device name. Can be used for searching and filtering devices.")
    image: Optional[StrictStr] = None
    tags: List[StrictStr]
    tenant_id: Annotated[str, Field(strict=True)] = Field(description="The UUID of the team owning the device(s)", alias="tenantId")
    sub_type: BLEDeviceSubType = Field(alias="subType")
    firmware: Optional[DeviceFirmwareArgs] = None
    meta: DeviceDocMeta = Field(alias="$meta")
    cloud_mqtt_enabled: Optional[StrictBool] = Field(default=False, description="This describes if the device has been fully onboarded to the cloud, and has the proper certificates to connect via MQTT. It may take up to 5 minutes for this to be updated after a certificate is registered to the cloud.", alias="cloudMqttEnabled")
    type: DeviceTypesBLE
    gateway_id: Annotated[str, Field(strict=True)] = Field(description="This is the canonical device id used in the device certificate, and as the MQTT client id.", alias="gatewayId")
    state: Optional[Dict[str, BLEDeviceStateValue]] = None
    __properties: ClassVar[List[str]] = ["id", "name", "image", "tags", "tenantId", "subType", "firmware", "$meta", "cloudMqttEnabled", "type", "gatewayId", "state"]

    @field_validator('id')
    def id_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^[a-z0-9:_-]{1,128}$", value ,re.IGNORECASE):
            raise ValueError(r"must validate the regular expression /^[a-z0-9:_-]{1,128}$/i")
        return value

    @field_validator('name')
    def name_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^(.){1,128}$", value):
            raise ValueError(r"must validate the regular expression /^(.){1,128}$/")
        return value
    ...
    def tenant_id_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^[a-f0-9]{8}-?[a-f0-9]{4}-?[a-f0-9]{4}-?[a-f0-9]{4}-?[a-f0-9]{12}$", value):
            raise ValueError(r"must validate the regular expression /^[a-f0-9]{8}-?[a-f0-9]{4}-?[a-f0-9]{4}-?[a-f0-9]{4}-?[a-f0-9]{12}$/")
        return value

    @field_validator('gateway_id')
    def gateway_id_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^[a-z0-9:_-]{1,128}$", value ,re.IGNORECASE):
            raise ValueError(r"must validate the regular expression /^[a-z0-9:_-]{1,128}$/i")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        _field_dict = {}
        if self.state:
            for _key_state in self.state:
                if self.state[_key_state]:
                    _field_dict[_key_state] = self.state[_key_state].to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "firmware": DeviceFirmwareArgs.from_dict(obj["firmware"]) if obj.get("firmware") is not None else None,
            "$meta": DeviceDocMeta.from_dict(obj["$meta"]) if obj.get("$meta") is not None else None,
            "cloudMqttEnabled": obj.get("cloudMqttEnabled") if obj.get("cloudMqttEnabled") is not None else False,
            "type": obj.get("type"),
            "gatewayId": obj.get("gatewayId"),
            "state": dict(
                (_k, BLEDeviceStateValue.from_dict(_v))
