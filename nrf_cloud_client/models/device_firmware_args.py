...
class DeviceFirmwareArgs(BaseModel):
    """
    DeviceFirmwareArgs
    """ # noqa: E501
    supports: Optional[List[StrictStr]] = None
    app: Optional[DeviceFirmwareArgsApp] = None
    ble_support: Optional[StrictBool] = Field(default=None, alias="bleSupport")
    boot: Optional[StrictStr] = None
    modem: Optional[StrictStr] = None
    smp_device_app_ver: Optional[StrictStr] = Field(default=None, alias="smpDeviceAppVer")
    __properties: ClassVar[List[str]] = ["supports", "app", "bleSupport", "boot", "modem", "smpDeviceAppVer"]

    @field_validator('supports')
    def supports_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        for i in value:
            if i not in set(['APP', 'MODEM', 'BOOT', 'SOFTDEVICE', 'BOOTLOADER', 'MDM_FULL', 'SMP', 'CUSTOM']):
                raise ValueError("each list item must be one of ('APP', 'MODEM', 'BOOT', 'SOFTDEVICE', 'BOOTLOADER', 'MDM_FULL', 'SMP', 'CUSTOM')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "app": DeviceFirmwareArgsApp.from_dict(obj["app"]) if obj.get("app") is not None else None,
            "bleSupport": obj.get("bleSupport"),
            "boot": obj.get("boot"),
            "modem": obj.get("modem"),
            "smpDeviceAppVer": obj.get("smpDeviceAppVer")
        })
        return _obj
