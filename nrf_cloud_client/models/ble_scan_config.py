...
class BLEScanConfig(BaseModel):
    """
    Parameters for configuring a BLE scan operation.
    """ # noqa: E501
    scan_type: Optional[BLEScanType] = Field(default=None, alias="scanType")
    scan_timeout: Annotated[int, Field(le=65535, strict=True, ge=1)] = Field(description="Scan timeout between `1` and `65535` seconds, `0` disables timeout. Default = `3`.", alias="scanTimeout")
    scan_reporting: Optional[StrictStr] = Field(default='instant', description="Use `batch` to send one message for all advertisements received on scan timeout. Use `instant` if every advertisement received shall be sent instantly. Default = `instant`.", alias="scanReporting")
    filter: Optional[BLEScanConfigFilter] = None
    __properties: ClassVar[List[str]] = ["scanType", "scanTimeout", "scanReporting", "filter"]

    @field_validator('scan_reporting')
    def scan_reporting_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['instant', 'batch']):
            raise ValueError("must be one of enum values ('instant', 'batch')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "scanTimeout": obj.get("scanTimeout"),
            "scanReporting": obj.get("scanReporting") if obj.get("scanReporting") is not None else 'instant',
            "filter": BLEScanConfigFilter.from_dict(obj["filter"]) if obj.get("filter") is not None else None
        })
        return _obj
