...
ACCOUNTINFOARGSPLAN_ANY_OF_SCHEMAS = ["DeveloperPlan", "EnterprisePerDeviceBillingModelPlan", "ProOrHigherPlan"]

class AccountInfoArgsPlan(BaseModel):
    """
    AccountInfoArgsPlan
    """

    # data type: DeveloperPlan
    anyof_schema_1_validator: Optional[DeveloperPlan] = None
    # data type: ProOrHigherPlan
    anyof_schema_2_validator: Optional[ProOrHigherPlan] = None
    # data type: EnterprisePerDeviceBillingModelPlan
    anyof_schema_3_validator: Optional[EnterprisePerDeviceBillingModelPlan] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[DeveloperPlan, EnterprisePerDeviceBillingModelPlan, ProOrHigherPlan]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "DeveloperPlan", "EnterprisePerDeviceBillingModelPlan", "ProOrHigherPlan" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = AccountInfoArgsPlan.model_construct()
        error_messages = []
        # validate data type: DeveloperPlan
        if not isinstance(v, DeveloperPlan):
            error_messages.append(f"Error! Input type `{type(v)}` is not `DeveloperPlan`")
        else:
            return v

        # validate data type: ProOrHigherPlan
        if not isinstance(v, ProOrHigherPlan):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ProOrHigherPlan`")
        else:
            return v

        # validate data type: EnterprisePerDeviceBillingModelPlan
        if not isinstance(v, EnterprisePerDeviceBillingModelPlan):
            error_messages.append(f"Error! Input type `{type(v)}` is not `EnterprisePerDeviceBillingModelPlan`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in AccountInfoArgsPlan with anyOf schemas: DeveloperPlan, EnterprisePerDeviceBillingModelPlan, ProOrHigherPlan. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[DeveloperPlan] = None
        try:
            instance.actual_instance = DeveloperPlan.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[ProOrHigherPlan] = None
        try:
            instance.actual_instance = ProOrHigherPlan.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_3_validator: Optional[EnterprisePerDeviceBillingModelPlan] = None
        try:
            instance.actual_instance = EnterprisePerDeviceBillingModelPlan.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into AccountInfoArgsPlan with anyOf schemas: DeveloperPlan, EnterprisePerDeviceBillingModelPlan, ProOrHigherPlan. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], DeveloperPlan, EnterprisePerDeviceBillingModelPlan, ProOrHigherPlan]]:
