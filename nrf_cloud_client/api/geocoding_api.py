...
class GeocodingApi:
    ...
    @validate_call
    def get_address_data_from_coordinates(
        self,
        ...
        Convert a set of WGS 84 coordinates into a physical address. Only country and state/province/region are guaranteed. Higher accuracy properties (street number, street, etc) are provided when available.  A 404 status code (not found) will be returned if nRF Cloud cannot determine at least country and state/province/region for the provided coordinates.  nRF Cloud returns the address data closest to the submitted coordinates. Coordinates from a higher-density urban areas generally provide better results. Coordinates from rural areas may return address data further away from the submitted coordinates. The system returns an uncertainty property, which is the distance in meters away from the submitted coordinates.  Coordinates obtained with a high-accuracy method (GNSS / Wi-Fi) will provide address data closer to the actual device location than coordinates obtained with lower-accuracy methods like cellular.  ```sh curl --location --request GET \"https://api.nrfcloud.com/v1/location/geo?lat=45.524098&lon=-122.688408\" \\ --header \"Authorization: Bearer $JWT\" \\ --header \"Content-Type: application/json\" ```
        ...
        _param = self._get_address_data_from_coordinates_serialize(
            lat=lat,
            lon=lon,
    ...
    @validate_call
    def get_address_data_from_coordinates_with_http_info(
        self,
        lat: Union[StrictFloat, StrictInt],
        lon: Union[StrictFloat, StrictInt],
        x_custom_device_id: Optional[Annotated[str, Field(strict=True)]] = None,
        ...
        _headers: Optional[Dict[StrictStr, Any]] = None,
        _host_index: Annotated[StrictInt, Field(ge=0, le=0)] = 0,
    ) -> ApiResponse[ReverseGeoResponse]:
        """GetAddressDataFromCoordinates
    ...
    @validate_call
    def get_address_data_from_coordinates_without_preload_content(
        self,
        lat: Union[StrictFloat, StrictInt],
        lon: Union[StrictFloat, StrictInt],
        x_custom_device_id: Optional[Annotated[str, Field(strict=True)]] = None,
    ...
    def _get_address_data_from_coordinates_serialize(
        self,
        lat,
        lon,
        x_custom_device_id,
