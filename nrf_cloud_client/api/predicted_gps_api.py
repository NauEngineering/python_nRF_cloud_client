...
class PredictedGPSApi:
    ...
    def get_predicted_assistance_data(
        ...
        """(Deprecated) GetPredictedAssistanceData

        <div style=\"background-color: #f8d7da; color: #721c24; text-align: center; border: 1px solid #f5c6cb; border-radius: .25rem; padding: .75rem 1.25rem\">This endpoint has been deprecated. Please use the <a href=\"#tag/GNSS/operation/GetPredictedAssistanceData\">GNSS GetPredicitonData endpoint</a> instead.</div>  Generates up to two weeks of gps predictions. The predictions are used by the nRF91 GPS modem to enable a faster time-to-first-fix (TTFF) and provide offline navigation when out of range of an LTE cell.  Returns a link for easy download of the predictions file.  __Basic Request__: ```sh curl -G $API_HOST/v1/location/pgps -H \"Authorization: Bearer $JWT\" -H \"Content-Type: application/json\" ``` __Custom Request__: ```sh curl -G $API_HOST/v1/location/pgps  -H \"Authorization: Bearer $JWT\" -H \"Content-Type: application/json\" -d \"predictionCount=42\" -d \"predictionIntervalMinutes=240\" -d \"startGpsDay=15131\" -d \"startGpsTimeOfDaySeconds=0\" ```
        ...
        """ # noqa: E501
        warnings.warn("GET /location/pgps is deprecated.", DeprecationWarning)
